...
def analyser_pedigree():
    ...
    try:
        ...
        # ETAPE 7 : Coherence Wright / ML
        # Si Wright dit ACCEPTABLE, le ML ne doit pas afficher RISQUE
        if niveau == 'ACCEPTABLE' and ml_resultat == 'RISQUE':
            ml_resultat             = 'ACCEPTABLE'
            ml_confiance_risque     = round(ml_confiance_risque * 0.2, 3)
            ml_confiance_acceptable = round(1 - ml_confiance_risque, 3)

        print(f"   [OK] Resultat: F={f_pourcent}% | {relation} | {niveau} ({methode})")
        print(f"   [ML] ML: {ml_resultat} (risque={ml_confiance_risque:.0%})")
